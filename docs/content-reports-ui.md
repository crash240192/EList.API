...
| Заблокировать аккаунт / организацию | `active=false` + запись в `moderation_penalties`; токены устройств **не** сбрасываются — после разбана повторная активация не нужна; при сроке вход/орг. вернутся сами |
...
| `SuspendAccount` | площадка | `accounts.active=false`; логин и API отклоняются с 20006; сессионные токены сохраняются |
