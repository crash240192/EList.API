...
```
Тариф платформы  →  Wallet / Tariffs      (внутренний баланс сервиса)
Билеты события   →  Orders + ЮKassa       (деньги организатора + комиссия сервиса)
```

В UI и аналитике не смешивать: пополнение кошелька ≠ покупка билета.
