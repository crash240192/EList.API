...
-- =============================================================================
-- Additional query-driven indexes (safe non-unique variants for existing DBs)
-- If UNIQUE is desired later, dedupe first then create *_uidx counterparts.
-- =============================================================================

CREATE INDEX IF NOT EXISTS accounts_login_idx
...
CREATE INDEX IF NOT EXISTS participations_account_event_idx
	ON public.participations (account_id, event_id);

CREATE INDEX IF NOT EXISTS subscriptions_pair_idx
	ON public.subscriptions (subscriber_id, subscribed_to_id);

CREATE INDEX IF NOT EXISTS invitations_event_invited_idx
	ON public.invitations (event_id, invited_id);

CREATE INDEX IF NOT EXISTS events_rating_event_voter_type_idx
	ON public.events_rating (event_id, rating_type, voter_id);

CREATE INDEX IF NOT EXISTS event_organizators_event_account_idx
	ON public.event_organizators (event_id, account_id)
	WHERE account_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS event_organizators_event_organization_idx
	ON public.event_organizators (event_id, organization_id)
	WHERE organization_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS participants_white_list_event_account_idx
	ON public.participants_white_list (event_id, account_id);

CREATE INDEX IF NOT EXISTS participants_black_list_event_account_idx
...
CREATE INDEX IF NOT EXISTS account_album_rls_account_album_idx
	ON public.account_album_rls (account_id, album_id);

CREATE INDEX IF NOT EXISTS event_album_rls_event_album_idx
...
CREATE INDEX IF NOT EXISTS account_agreement_account_document_idx
	ON public.account_agreement_rls (account_id, document_id);

CREATE INDEX IF NOT EXISTS organization_agreement_org_document_idx
